...
function CurrencyCard({ currency, onOpen }: { currency: Currency; onOpen: (c: Currency) => void }) {
  ...
  useEffect(() => {
    if (!isVisible) return;
    // Defer to next paint so the transition animates from 0%
    const id = requestAnimationFrame(() => setFillValue(currency.weight));
    return () => cancelAnimationFrame(id);
  }, [isVisible, currency.weight]);
  ...
  return (
    ...
    >
      <Progress
        ...
        radius="default"
        showText
        overlayClassName="flex items-stretch"
        className="aspect-[5/4] bg-slate-950/70 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
        indicatorClassName={`bg-gradient-to-b ${currency.color}`}
      >
        <div className="flex flex-col justify-between items-start text-white w-full h-full gap-4">
          <div className="flex items-center gap-3">
            <span className="text-2xl sm:text-3xl font-black leading-none drop-shadow">{fillValue.toFixed(0)}%</span>
            <span className="text-[10px] uppercase tracking-[0.22em] font-semibold opacity-80">Basket</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-medium opacity-80 leading-tight">{currency.name}</span>
            </div>
          </div>
        </div>
      </Progress>
    ...
    <AnimatePresence>
      {currency && (
        ...
        >
          ...
          />
          <motion.div
            className="relative z-10 w-full max-w-lg rounded-3xl bg-slate-950/90 border border-white/10 shadow-2xl overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center justify-between p-5">
              <div className="flex items-center gap-3">
                <span className="text-2xl">{currency.country}</span>
              ...
              <button
                type="button"
                onClick={onClose}
                className="text-white/70 hover:text-white rounded-full p-2 bg-white/5 border border-white/10"
                aria-label="Close"
              >
                ✕
              </button>
            ...
            <div className="px-5 pb-5 flex flex-col gap-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 text-white">
                  <div className="text-sm uppercase tracking-[0.18em] text-white/60">Basket Weight</div>
