...
export const DownloadButton = ({ label = 'Download', i18nKey = 'download.button' }: DownloadButtonProps) => {
  ...
  const handleClick = () => {
    ...
    // Open modal immediately and start animation
    setIsModalOpen(true);
    setIsDownloading(true);

    // Keep animation running, don't stop it
    // Animation will stop when modal is closed
  };
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {!isDownloading && (
            ...
            >
              <Trans i18nKey={i18nKey} fallback={label} />
            </motion.span>
          )}
        </AnimatePresence>
      </motion.button>
